...
public class ConsumerController {
    ...
    public class MyPicConfig implements WebMvcConfigurer {
        ...
        public void addResourceHandlers(ResourceHandlerRegistry registry) {
            registry.addResourceHandler("/avatorImages/**").addResourceLocations("file:" + musicData + "/avatorImages/");
        }
    }
...
//    更新用户头像
    ...
    @PostMapping("/admin/updateSwiper")
    public Object updateSwiper(@RequestParam("file") MultipartFile swiperFile, @RequestParam("id")int id) {
        JSONObject jsonObject = new JSONObject();

        if (swiperFile.isEmpty()) {
            jsonObject.put("code", 0);
            jsonObject.put("msg", "文件上传失败！");
            return jsonObject;
        }
        ...
        File dest = new File(filePath + System.getProperty("file.separator") + fileName);
        String storeSwiperPath = "/avatorImages/"+fileName;
        try {
            swiperFile.transferTo(dest);
            int res = consumerMapper.updateSwiper(id, storeSwiperPath);
            if (res == 1){
                jsonObject.put("code", 1);
                jsonObject.put("swiper", storeSwiperPath);
                jsonObject.put("msg", "上传成功");
                return jsonObject;
            }else {
                jsonObject.put("code", 0);
