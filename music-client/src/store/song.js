const song = {
  ...
  getters: {
    ...
    voiceSize: state => {
      let voiceSize = state.voiceSize
      if (!voiceSize) {
        voiceSize = JSON.parse(window.sessionStorage.getItem('voiceSize') || null)
      }
      return voiceSize
    }
  },
  mutations: {
    ...
    setVoiceSize: (state, voiceSize) => {
      state.voiceSize = voiceSize
      window.sessionStorage.setItem('voiceSize', JSON.stringify(voiceSize))
    }
  },
  actions: {}
}
